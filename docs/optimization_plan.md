...
**实现**：
- 后端：新增 `/api/doc?path=...` endpoint，读取本地 MD → marked 渲染 HTML
...
## 五-OCTO、Query Rewriting 查询改写

### 5H.1 当前问题

用户输入往往是口语化的，如"agent开发只用python行不行"，直接检索可能命中率低——关键词匹配找不到"Python Agent开发"、"Agent技术栈"等正式表述，语义搜索也可能被口语噪音干扰。

### 5H.2 方案

在检索之前插入 LLM 改写步骤，LLM 自行判断输入类型并选择策略：

```
用户问题 → LLM 分析类型(type: decompose|oral|fuzzy)
         → LLM 改写为 2-3 条搜索 query
         → 多条 query 并行检索
         → RRF 融合去重 → 最终候选集
         → 原始question + 检索结果 → LLM 生成答案
```

改写后的 query 仅用于提升检索召回，最终生成答案时仍使用用户的原始问题。

**LLM 自判断三种策略**：

| 类型 | 触发条件 | 策略 | 示例 |
|------|---------|------|------|
| `decompose` | 长句、多个子话题 | 拆解为 2-3 条独立 query | "Agent面试考什么，大模型八股文也要" → ["Agent面试 常见问题", "大模型 八股文 面试题"] |
| `oral` | 口语化、非正式表达 | 标准化为搜索关键词 | "agent开发只用python行不行啊" → ["Agent开发 Python语言 技术栈选择", "Agent开发 是否需要Java"] |
| `fuzzy` | 模糊、缺少专业术语 | 生成同义精准 query | "那个什么推理的pattern" → ["ReAct模式 Agent推理", "Reflection模式 Agent推理框架"] |

**Rewrite Prompt**：

```
你是一个查询改写专家。分析用户输入，判断类型并改写：

1. 如果句子长、包含多个子话题 → type=decompose，拆解为2-3条独立query
2. 如果口语化/非正式 → type=oral，标准化为搜索关键词
3. 如果表达模糊、缺少专业术语 → type=fuzzy，生成同义精准query

只输出JSON: {"type":"decompose|oral|fuzzy","queries":["q1","q2"]}
```
...
改写输出 3 条 query
  ├─ query1 → semantic → 结果1 ─┐
  ├─ query2 → semantic → 结果2 ─┼─ RRF 融合去重 → 最终 top-K
  └─ query3 → semantic → 结果3 ─┘
```
...
```
rewrite    120ms — type=oral, "agent开发只用python行不行" → 2条query
  ├─ semantic    50ms  — 候选 15 篇
  └─ rerank     150ms  — 精排后 top-5
LLM         2000ms (上下文 3247字→486字)
```

### 5H.4 Trace 可见

改写步骤作为 Span 出现在 Trace 面板：

```
rewrite    120ms — "agent开发只用python行不行" → "AI Agent开发 Python语言 技术栈"
  ├─ semantic    50ms  — 候选 20 篇
  └─ rerank     150ms  — 精排后 top-5
LLM         2000ms (上下文 3247字→486字)
```
...
| 文件 | 改动 | 工时 |
|------|------|------|
| `src/kb_agent/rewriter.py`（新） | QueryRewriter 类，调用 LLM 改写 query | ~0.5h |
| `src/kb_agent/qa.py` | answer_question 检索前调用 rewriter | ~0.25h |
| `src/config.py` | 新增 `rewrite_enabled: bool = True` | ~5min |
| `webui/server.py` | _ask() 端点透传改写结果 | ~0.1h |
| `webui/index.html` | Trace 面板展示改写步骤 | ~0.1h |

### 5H.6 预估

- 工时：~1h
- 新增依赖：无
- 额外 API 调用：每次问答多 1 次 LLM 调用（轻量，只要求输出少量文本）
- 收益：关键词检索召回率预期提升 10-20%
...
| **P11** | Query Rewriting 查询改写 | ~1.5h | 新增 rewriter.py, qa.py, config.py, webui | LLM自判断(decompose/oral/fuzzy)改写，并行检索合并 |
...
**第十一阶段（1.5h）**：Query Rewriting 查询改写
- LLM 自判断类型（长句拆解/口语标准化/模糊扩写）→ 多 query 并行检索合并
